...
def add_database(data):
    ...
    with open(file="file/databases.txt", mode="r", encoding="utf-8") as r_f:
        ...
        '''
        :param 获取post发送过来的数据，组成一个新的dict
        '''
        ...
        for n in range(len(database_info)):
            ...
            # 判断 数据库中是否已经存在
            if database_info[n].get("client") == database_dict.get("client"):
                continue

            else:

                # 数据库写入操作
                # database_info[n]["id"] = n
...
def health():
    ...
    with open(file='file/databases.txt', mode='r', encoding='utf-8') as r:
        ...
        for i in range(len(a)):
            ...
            try:
                cli = b['client']
                requests.get("http://%(ip)s:32373/health" % {'ip': cli}, timeout=2).text
